...
const createCategory = (data) => {
	...
	document.getElementById("pack-selector-container1.16").appendChild(div)
...
modulesxobj.open("GET", "/api/modules/1.14", true)
modulesxobj.onreadystatechange = function () {
	if (modulesxobj.readyState == 4 && modulesxobj.status == "200") {
		...
		categoriesxobj.open("GET", "/api/categories/1.14", true)
		categoriesxobj.onreadystatechange = function () {
			if (categoriesxobj.readyState == 4 && categoriesxobj.status == "200") {
				...
				const filledCategories = Array.from(document.getElementById("pack-selector-container1.16").children)
